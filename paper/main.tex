...
\textbf{Pipeline scripts.} Each analytical step is a self contained Python script that reads only the parquet feature matrix and the labels (never the raw text) and emits a JSON or parquet artifact:
{\small
\begin{itemize}\itemsep0pt\parskip0pt\leftmargin=*
\item \texttt{run\_baselines.py} $\rightarrow$ \texttt{baselines.json}
\item \texttt{run\_ablation.py} $\rightarrow$ \texttt{ablation.json}
\item \texttt{run\_temporal\_drift.py} $\rightarrow$ \texttt{temporal\_\allowbreak drift.json}
\item \texttt{run\_interactions.py} $\rightarrow$ \texttt{interactions.json}
\item \texttt{run\_validation\_kappa.py} $\rightarrow$ \texttt{validation\_\allowbreak kappa.json}
\item \texttt{multi\_model\_study.py} $\rightarrow$ \texttt{multi\_model/\allowbreak multi\_model\_study.json}
\end{itemize}
}
All artifacts land under \texttt{results/} and can be regenerated without re-fetching the corpus.
